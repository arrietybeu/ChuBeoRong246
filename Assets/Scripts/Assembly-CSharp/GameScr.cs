        ...
        try
        {
            ...
            for (int i = 0; i < num; i++)
			{
                ...
                for (int j = 0; j < parts[i].pi.Length; j++)
				{
                    ...
                    img["icon_id"] = parts[i].pi[j].id;
                    img["dx"] = parts[i].pi[j].dx;
                    img["dy"] = parts[i].pi[j].dy;
                    dataList.Add(img);

                }

                partDict["data"] = dataList;
                jsonList.Add(partDict);

            }
            File.WriteAllText("part.json", Newtonsoft.Json.JsonConvert.SerializeObject(jsonList, Newtonsoft.Json.Formatting.Indented));
